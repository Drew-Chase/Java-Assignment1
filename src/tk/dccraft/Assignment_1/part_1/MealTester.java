...
public class MealTester {
	...
	private void initOrderWindow() {
		...
		for (String s : menu.keySet()) {
			for (double d : menu.values()) {
				button[menu.size()] = new JButton(new String(s + " = " + d));
				button[menu.size()].setSize(new Dimension(100, 50));
				print("Added UI Element: " + s + " Button");
			}
		}
		for (int i = 0; i < menu.size(); i++) {
			orderWindow.add(button[i]);
		}
